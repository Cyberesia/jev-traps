...
export function IntegrationGuide() {
  ...
  return <><div className="tabs" role="group" aria-label="Agent provider">{labels.map(p=><button className={`pill ${p===provider ? "active" : ""}`} key={p} aria-pressed={p===provider} onClick={()=>setProvider(p)}>{p}</button>)}</div><pre className="code">{`import { createTraps, requireAllowed } from "@jev-traps/sdk";\n\nconst traps = createTraps({\n  semantic: true,\n  destination: { provider: localFeedProvider },\n  registry: {\n    endpoint: "https://your-registry.example",\n    apiKey: process.env.JEV_TRAPS_REGISTRY_KEY!\n  }\n});\n// Check before navigation. API keys stay on your server.\nconst destination = await traps.preflightUrl(targetUrl);\nif (destination.action !== "proceed")\n  throw new Error(destination.reason);\nconst untrustedText = await retrievalTool(targetUrl);\nconst report = await traps.inspectText(untrustedText, {\n  goal: "Summarize today's support incidents",\n  url: targetUrl\n});\nrequireAllowed(report); // throws on sanitize / review / block\n\n${returns[provider]}`}</pre><p className="note">Host middleware pattern, not a complete provider API client. Destination matches warn or stop the host and are never auto-published. Non-allow content results with a public URL publish only minimal metadata to the configured Registry; raw content and evidence never leave through that channel. Reporting failure never weakens local enforcement.</p></>;
}

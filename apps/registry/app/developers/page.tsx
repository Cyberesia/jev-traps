...
export default function Developers() { return <div className="shell"><SiteNav /><main><header className="dev-header"><div className="eyebrow">SDK / integration guide</div><h1>Your agent. Your tools.<br /><em>An explicit boundary.</em></h1><p className="lead">Check a destination before navigation. If retrieval proceeds, inspect its content before the next model call and enforce both results in your host.</p></header><div className="dev-grid"><section className="panel wide"><h2>01 / Connect your agent</h2><IntegrationGuide /></section><section className="panel wide"><h2>02 / Know exactly where data goes</h2><div className="table-wrap"><table className="data-table"><thead><tr><th>Operation</th><th>Data leaving your process</th><th>Recipient</th><th>Registry involved?</th></tr></thead><tbody><tr><td>Local destination preflight</td><td>None; URL matched against a local snapshot</td><td>Local code</td><td>No</td></tr><tr><td>Optional Jev destination assessment</td><td>URL + hostname only</td><td>TypeSafe / configured Jev model</td><td>No</td></tr><tr><td>Static text / HTML scan</td><td>None</td><td>Local code</td><td>No</td></tr><tr><td>Jev semantic scan</td><td>Candidate text, task context, static signals</td><td>TypeSafe / configured Jev model</td><td>No</td></tr><tr><td>Image inspection</td><td>Image bytes + context; then extracted observations</td><td>Your vision provider; then TypeSafe</td><td>No</td></tr><tr><td>Registry lookup</td><td>Public URL queried</td><td>This registry host</td><td>Lookup only; never a scan</td></tr><tr><td>SDK automated observation</td><td>Normalized URL, action, risk, trap types, detector version, surface (non-allow + public URL only; never raw content)</td><td>This registry host /api/observations</td><td>Yes — private, unverified inbox</td></tr><tr><td>Community report</td><td>Public URL + your note</td><td>This host’s private review queue</td><td>No automatic publication</td></tr></tbody></table></div></section><section className="panel" id="vision"><h2>03 / Inspect the whole visual frame</h2><pre className="code">{`import { readFile } from "node:fs/promises";
